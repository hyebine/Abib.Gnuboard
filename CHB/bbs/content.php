...
if(is_file($skin_file)) {
    ...
    $himgbg = G5_DATA_URL.'/content/'.$co_id.'_h';
    if (file_exists($himg)) // 상단 이미지
        echo run_replace('content_head_image_html', '<div id="ctt_himg" class="ctt_img d-flex align-items-center justify-content-center '.$co_id.'" style="background:url('.$himgbg.') no-repeat center; "><h2 id="container_title" class="mb-0 d-none"><span title="'.get_text($g5['title']).'">'.get_head_title($g5['title']).'</span></h2><span class="text">Meet with the first moon.<br/>
        Renew yourself as you meet Abib.</span></div>', $co);

    include($skin_file);

    
if ($is_admin)
echo run_replace('content_admin_button_html', '<div class="ctt_admin"><a href="'.G5_ADMIN_URL.'/contentform.php?w=u&amp;co_id='.$co_id.'" class="btn_admin btn"><span class="">내용 수정</span></a></div>', $co);
